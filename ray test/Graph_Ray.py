...
@ray.remote(num_gpus=7)
class ng:
    def __init__(self, feature_set, gpu, vertex_nums=1200, alpha = 10, eta = 0.1, max_iter = 100, batch_size = 100):
        self.xp = None
    ...
    #function for ray (parallel)
    def compute_rank_order_batch(self,f_batch, anchor_set):
        Rf_refs = []
        for i in range(self.BATCH_SIZE):
            Rf_refs.append(self.compute_rank_order.remote(f_batch[i], anchor_set))
        return ray.get(Rf_refs)
    ...
    def update_anchor_set(self, f_set, anchor_set):
        for f_idx in tqdm(range(0, f_set.shape[0], self.BATCH_SIZE), desc="update anchor (batch)"):
            f_batch = f_set[f_idx : f_idx + self.BATCH_SIZE]
            Rf_batch = self.compute_rank_order_batch(f_batch, anchor_set)

            for b_idx in range(self.BATCH_SIZE):
                Rf = Rf_batch[b_idx]
                f = f_batch[b_idx]
                anchor_refs = self.anchor_set
                for i in range(self.max_iter):
                    # Todo 이거 max iter수가 100~1000인데 inc에서는 m이 그만큼 안많아서 예외처리
                    if len(Rf) < self.max_iter:
                        break
                    m_idx = Rf[i]
                    m = self.anchor_set[m_idx]
                    anchor_set[m_idx] = ray.get(self.update_anchor.remote(f,m,i))
