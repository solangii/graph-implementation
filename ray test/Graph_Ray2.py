...
class ng:
    ...
    def update_anchor(self, feature, anchor, k, t, t_max):
        """
        update anchor(m) by equation3 in FSCIL.
        :param feature: each feature
        :param anchor: centroid
        :param k: iterate number (k th nearest m vector)
        :param t : t th feature
        :return updated anchor position
        """
        # rate = self.eta * self.xp.exp(-i/self.alpha)
        epsilon_t = self.epsilon_i * self.xp.power((float(self.epsilon_f / self.epsilon_i)), float(t / t_max))
        lamb_t = self.lambda_i * self.xp.power(float(self.lambda_f / self.lambda_i), float(t / t_max))
        h_lamb_k = self.xp.exp(float(-k / lamb_t))
        rate = epsilon_t * h_lamb_k
    ...
    def update_anchor_set(self, f_set, anchor_set):
        ...
        for f_idx in tqdm(range(f_set.shape[0]), desc="update anchor"):
            ...
            for k in range(self.max_iter):
                ...
                anchor_set[m_idx] = self.update_anchor(f, m, k, f_idx, f_set.shape[0])
    ...
    def pretrain(self, x,y):
        ...
        :return:
        """
        self.anchor_set = self.update_anchor_set.remote(self.feature_set, self.anchor_set)
        print("anchor update done!")
        print("============================================================")

        self.vertices = self.make_vertex.remote(self.anchor_set, self.vertex_nums)
        print("make vertex done!")

        self.vertices = self.update_vertex.remote(x, y, self.vertices, self.feature_set, self.anchor_set)
